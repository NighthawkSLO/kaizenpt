...
internal static class Utils
{
	...
	internal static object? GetProperty(object receiver, string propertyName)
	{
		Type type = receiver.GetType();
		PropertyInfo property =
			type.GetProperty(propertyName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
			?? throw new FindMemberException(
				$@"Failed to find property ""{propertyName}"" in type ""${type.AssemblyQualifiedName}"""
			);
		return property.GetValue(receiver);
	}

	internal static void SetProperty(object receiver, string propertyName, object? value)
	{
		Type type = receiver.GetType();
		PropertyInfo property =
			type.GetProperty(propertyName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
